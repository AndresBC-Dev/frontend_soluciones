...
const GestionEvaluacionesPage: React.FC = () => {
    ...
    const loadData = async () => {
        ...
        try {
            ...
            // Ordenar períodos por fecha de creación descendente (más recientes primero)
    ...
    const handleDeletePeriod = (period: Period) => {
        // Verificar si el período ya está inactivo
        if (!period.is_active) {
            showConfirmation(
                'Período ya inactivo',
    ...
    • Podrá reactivarlo desde el modo de edición`,
            async () => {
                ...
                try {
                    // Opción 1: Si el backend maneja el soft delete con DELETE
                    await deletePeriod(period.id);

                    // Opción 2: Actualizar el período en lugar de removerlo de la lista
                    // Si quieres mantenerlo visible pero marcado como inactivo:
                    /*
                    const deactivatedPeriod = { ...period, is_active: false };
                    setPeriods(prev => prev.map(p => 
                      p.id === period.id ? deactivatedPeriod : p
                    ));
                    */

                    // Opción 3: Remover de la lista (más limpio para el usuario)
