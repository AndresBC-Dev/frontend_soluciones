...
const EditarPeriodoModal: React.FC<EditarPeriodoModalProps> = ({
  ...
  const validateForm = (): string | null => {
    if (!period?.can_modify) {
      return 'No se puede modificar este período porque está finalizado o expirado.';
    }
    ...
    if (form.isActive && dueDate < today) {
      return 'No se puede activar un período con una fecha límite vencida. Por favor, actualiza las fechas a futuras.';
    }
    ...
    try {
      ...
      if (statusChanged) {
        if (form.isActive) {
          ...
          if (updatedPeriod.status !== 'draft') {
            console.log('📝 Setting to draft before activation...');
            try {
              updatedPeriod = await updatePeriod(period.id, { status: 'draft' });
              console.log('✅ Period set to draft:', updatedPeriod);
            } catch (draftError) {
              console.error('❌ Failed to set period to draft:', draftError);
              throw new Error('No se pudo establecer el período en estado borrador.');
            }
          }
          console.log('🔄 Attempting to activate period...');
          updatedPeriod = await activatePeriod(period.id);
          console.log('✅ Period activated:', updatedPeriod);
        } else {
          console.log('🔄 Deactivating period...');
          updatedPeriod = await deactivatePeriod(period.id);
          console.log('✅ Period deactivated:', updatedPeriod);
        }
      }
      ...
      if (err.message.includes('no se puede modificar un período en estado completed')) {
        setError(
          'No se puede modificar este período porque está finalizado. Por favor, crea un nuevo período.'
        );
      } else if (err.message.includes('solo se pueden activar períodos en estado draft')) {
        setError(
          'No se puede activar este período porque no está en estado borrador. Por favor, intenta de nuevo.'
        );
      } else {
        setError(err.message || 'Error al actualizar el período. Por favor, intenta de nuevo.');
      }
    } finally {
      setLoading(false);
    }
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            ...
            <input
              ...
              disabled={loading || !(period.can_modify ?? true)}
              required
            />
          </div>
