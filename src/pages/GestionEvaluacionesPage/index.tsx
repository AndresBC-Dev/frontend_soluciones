...
import GenerarEvaluacionDesdeTemplateModal from '../../components/GenerarEvaluacionDesdeTemplateModal';
...
const GestionEvaluacionesPage: React.FC = () => {
    ...
    const [showGenerarEvaluacionModal, setShowGenerarEvaluacionModal] = useState(false);
    const [templateForEvaluation, setTemplateForEvaluation] = useState<TemplateListItem | null>(null);
    ...
    const handleGenerateEval = (template: Template) => {
        ...
        setTemplateForEvaluation(templateListItem);
        setShowGenerarEvaluacionModal(true);
    };
    ...
    return (
        <div className="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50 to-indigo-100">
            ...
            />
            <GenerarEvaluacionDesdeTemplateModal
                show={showGenerarEvaluacionModal}
                template={templateForEvaluation}
                onClose={() => {
                    setShowGenerarEvaluacionModal(false);
                    setTemplateForEvaluation(null);
                }}
                onCreated={(result) => {
                    loadData(); // Recargar evaluaciones
                    showConfirmation(
                        'Evaluaciones Creadas',
                        `Se han creado ${result.created_count || result.employee_ids?.length || 0} evaluaciones exitosamente.`,
                        () => hideConfirmation(),
                        'success'
                    );
                }}
